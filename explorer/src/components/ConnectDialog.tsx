import { useCreateConnection, useSearch } from '@aredotna/react-query'
import { type Channel, ConnectableType, SearchScope } from '@aredotna/sdk/api'
import { MagnifyingGlassIcon } from '@radix-ui/react-icons'
import { Dialog, Flex, ScrollArea, Spinner, Text, TextField } from '@radix-ui/themes'
...
export function ConnectDialog({
  ...
  const handleSelectChannel = (channel: Channel) => {
    ...
    createConnection.mutate({
      ...
      connectable_type:
        connectableType === 'block' ? ConnectableType.BLOCK : ConnectableType.CHANNEL,
  ...
  const channels = data?.data?.filter((item): item is Channel => item.type === 'Channel')
        ...
        <Flex direction="column" gap="3">
          ...
          <ScrollArea style={{ height: '300px' }} scrollbars="vertical">
            <Flex direction="column" gap="1">
              ...
              {searchQuery.length > 0 && !isSearching && channels?.length === 0 && (
                <Flex py="4" justify="center">
                  <Text size="2" color="gray" align="center">
                    No channels found
                  </Text>
              ...
              {channels?.map((channel) => (
                <ChannelLink
                  key={channel.id}
                  channel={channel}
                  onSelect={createConnection.isPending ? undefined : handleSelectChannel}
                />
              ))}
