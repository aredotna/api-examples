import { arenaQueryKeys, useCreateUploadBlock } from '@aredotna/react-query'
...
export function ChannelUploadDropzone({
  ...
  const activeCount = useMemo(
    () =>
      uploadItems.filter(
        (item) =>
          item.status === 'queued' || item.status === 'uploading' || item.status === 'creating',
      ).length,
    [uploadItems],
  )
  ...
  const uploadFile = useCallback(
      ...
      try {
        const block = await createUploadBlock.mutateAsync({
          ...
          onProgress: (sent, total) => {
            ...
            const progress = knownTotal > 0 ? Math.round((sent / knownTotal) * 100) : 100
    ...
    [channelNumericId, createUploadBlock, refreshChannel, updateUploadItem],
  )
  ...
  const startUploads = useCallback(
    (files: File[]) => {
      ...
      setUploadItems((current) => [...uploads.map(({ item }) => item), ...current])

      void Promise.allSettled(uploads.map(({ file, item }) => uploadFile(item.id, file)))
    },
    [uploadFile],
  )
  ...
  const uploadProgressContent = (
    <Flex direction="column" gap="3">
      ...
      <Flex direction="column" gap="3" style={{ maxHeight: '50vh', overflowY: 'auto' }}>
        {uploadItems.map((item) => (
          <Box key={item.id}>
            ...
            <Flex justify="between" gap="3" mt="1">
              <Text size="1" color={item.status === 'error' ? 'red' : 'gray'}>
  ...
  return (
    <>
      {(isDraggingFiles || uploadItems.length > 0) && (
        ...
        >
          <Card size="4" style={{ width: isDraggingFiles ? undefined : 'min(520px, 90vw)' }}>
            {isDraggingFiles ? (
